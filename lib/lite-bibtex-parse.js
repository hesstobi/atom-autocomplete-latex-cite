...
class BibtexParser {
  ...
  single_value() {
    ...
    else if (k.toLowerCase() in this.strings)
      return this.strings[k.toLowerCase()]
  ...
  key_equals_value() {
    ...
    throw new TypeError("Value expected, equals sign missing: key_equals_value" + this.key() + this.key().length,
                     this.input.substring(this.pos))
  }
  ...
  string() {
    // Add the current string to the BibtexParser strings.
    let {key, val} = this.key_equals_value()
    if (key != "empty")
  ...
  alernativeid() {
    this.entries.forEach(function(entry) {
      if (!entry.id && (entry.author || entry.year)) {
        entry.id = ''
        if (entry.author) {
          entry.id += entry.author.indexOf(', ') !== -1 ? entry.author.split(',')[0] : entry.author
          entry.id += ', '
        }
        entry.id += entry.year
      }
  ...
  parse() {
    while (this.matchAt()) {
      ...
      if (d.toUpperCase() === "@STRING") {
        ...
        this.comment()
      } else if (d.toUpperCase() === "@FOOTNOTE") {
          this.comment()
      } else if (d.toUpperCase() === "@CONTROL") {
          this.comment()
      } else {
        this.entry(d)
      }
